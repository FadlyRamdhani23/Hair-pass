...
class InputDataActivity : AppCompatActivity() {
    ...
    private fun setInitLayout() {
       ...
       btnJam.setOnClickListener { view: View? ->

            val jamJemput = Calendar.getInstance()
            val time =
                TimePickerDialog.OnTimeSetListener { view1: TimePicker?, hourOfDay: Int, minute: Int ->
                    jamJemput[Calendar.HOUR_OF_DAY] = hourOfDay
                    jamJemput[Calendar.MINUTE] = minute
                    val strFormatDefault = "HH:mm"
                    val simpleDateFormat = SimpleDateFormat(strFormatDefault, Locale.getDefault())
                    inputJam.setText(simpleDateFormat.format(jamJemput.time))
                }
            TimePickerDialog(
                this@InputDataActivity, time,
                jamJemput[Calendar.HOUR_OF_DAY],
                jamJemput[Calendar.MINUTE],
                true
            ).show()
        }
    ...
    private fun setInputData() {
        ...
        btnCheckout.setOnClickListener { v: View? ->
            ...
            strAlamat = inputJam.text.toString()
